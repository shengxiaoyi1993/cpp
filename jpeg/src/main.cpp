...
#include "../lib/jpeg-compressor/stb_image.c"
...
void test_RGB2JPEG(){
  string file_src="../resources/binary_mc_copy_8.ppm";
  string file_dst_gray="../resources/binary_mc_copy_gray.jpeg";
  string file_dst_rgb="../resources/binary_mc_copy_rgb.jpeg";
  string file_ppm_255_dst="../resources/file_ppm_255_dst.ppm";

  string file_pgm_255_dst="../resources/file_pgm_255_dst.ppm";


  PNM pnm(file_ppm_255_dst);
  pnm.print();
  pnm.saveToFile(file_pgm_255_dst,PNM::MagicNumber_p5,255);

  unsigned char* pdata=new unsigned char[pnm._width*pnm._height*3];
  for(size_t i=0;i<pnm._width*pnm._height;i++){
    pdata[i*3]=pnm._data[i*3]>>8;
    pdata[i*3+1]=pnm._data[i*3+1]>>8;
    pdata[i*3+2]=pnm._data[i*3+2]>>8;
  }

//  unsigned char* pdata=new unsigned char[pnm._width*pnm._height];
//  for(size_t i=0;i<pnm._width*pnm._height;i++){
//    pdata[i]=pnm._data[i]>>8;
//  }

  compress_image_to_jpeg_file(file_dst_rgb.c_str(),
                              pnm._width,
                              pnm._height,
                              3,
                              pdata
...
void test_JPEG2RGB(){
  string file_src="../resources/binary_mc_copy_8.ppm";
  string file_jpeg="../resources/binary_mc_copy.jpeg";
  string file_jpeg_decode="../resources/binary_mc_copy_jpg_decode.ppm";
  string file_ppm_255_dst="../resources/file_ppm_255_dst.ppm";
  ...
  unsigned char* pdata= jpgd::decompress_jpeg_image_from_file(file_jpeg.c_str(),
                                                        &width,
                                                        &height,
                                                        &actual_comps,
                                                        req_comps
  ...
  unsigned short* ppnm=new unsigned short[width*height*3];
  for(size_t i=0;i<width*height*3;i++){
    ppnm[i]=static_cast<unsigned short>(pdata[i])<<8;
  }

  int flag_save =PNM::saveDataToFile(file_jpeg_decode,PNM::DataType_rgb,PNM::MagicNumber_p6,ppnm,width,height,255);
  cout<<"flag_save:"<<flag_save<<endl;
  delete []ppnm;
