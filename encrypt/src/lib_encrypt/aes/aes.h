...
// enum KeySize { Bits128, Bits192, Bits256 };  // key size, in bits, for
// construtor
#define Bits128 16
#define Bits192 24
#define Bits256 32
...
class Aes  // Advanced Encryption Standard
{
 public:
  ...
  void Cipher(unsigned char* input,
              unsigned char* output);  // encipher 16-bit input 加密16-bit的输入
  void InvCipher(
      unsigned char* input,
      unsigned char* output);  // decipher 16-bit input 解密16-bit的输入
 private:
...
class TAesClass {
 public:
  ...
  DWORD OnAesEncrypt(LPVOID InBuffer, DWORD InLength,
                     LPVOID OutBuffer);  // AES 加密数据
  DWORD OnAesUncrypt(LPVOID InBuffer, DWORD InLength,
                     LPVOID OutBuffer);  // AES 解密数据
 private:
  Aes* m_lpAes;
};
