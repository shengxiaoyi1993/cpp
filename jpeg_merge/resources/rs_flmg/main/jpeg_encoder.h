...
public:
	...
	bool encodeToJPGbw(const char* fileName, int quality_scale);
...
private:
	...
	void _foword_FDC(const char* channel_data, float* fdc_data);
	void _foword_LFDC(const char* channel_data, float* fdc_data);
	...
	void _doHuffmanEncodingAll(const short* DU, short& prevDC, const BitString* HT, int isCrCb,
		BitString* outputBitString, int& bitStringCounts);

private:
	void _write_jpeg_header(FILE* fp);
	void _write_jpeg_header_bw(FILE* fp);
	...
	void _write_bytealign_(int& newByte, int& newBytePos, FILE* fp);
