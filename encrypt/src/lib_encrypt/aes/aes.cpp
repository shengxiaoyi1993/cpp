...
static unsigned char AesSbox[16 * 16] = {  // populate the Sbox matrix
...
Aes::Aes(int keysize, unsigned char* keyBytes) {
  ...
  memcpy(key, keyBytes,
         keysize);  //字符串拷贝函数，把keyBytes的keysize个字符复制到key中
...
unsigned char* Aes::SubWord(unsigned char* word) {
  ...
  for (int j = 0; j < 4; j++) {
    temp[j] =
        AesSbox[16 * (word[j] >> 4) +
                (word[j] & 0x0f)];  //实际上也可以写成AesSbox[[j]];因为两者相等
  }
...
void Aes::Cipher(unsigned char* input, unsigned char* output) {
  ...
  for (int i = 0; i < 4 * Nb;
       i++)  //这里是先写列后写行的，即输入是一列一列的进来的
  {
    State[i % 4][i / 4] =
        input[i];  //换成先写行后写列也是可以的，只要在输出时也是这样就可以了
  }
  AddRoundKey(0);  //轮密钥加
...
void Aes::AddRoundKey(int round) {
  ...
  {                          //                              k2 k6 k10k14
    ...
    {  // 所以i行j列的下标是4*((round*4)+j)+i即16*round+4*j+i
      State[i][j] =
          (unsigned char)((int)State[i][j] ^ (int)w[4 * ((round * 4) + j) + i]);
    }
  }
}
////////////////////////////////////////////////////////////////////////////////////////////////
...
void Aes::InvShiftRows() {
  ...
  for (i = 1; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      // if(i==1)State[i][j]=temp[4*i+(j-1)%4];    在此犯了一个错误 -1%4=-1
      // 而不是3，所以采用了下面再加一个4的做法
      if (i == 1)
...
DWORD TAesClass::OnAesEncrypt(LPVOID InBuffer, DWORD InLength,
                              LPVOID OutBuffer) {
  DWORD OutLength = 0;
  ...
  *((LPDWORD)extrabuff) =
      16 + (16 - leftnum) % 16;  //多出16+(16-leftnum)%16个字节
...
// 最近修改：2009 年 08 月 07 日
//------------------------------------------------------------------------------------------------------------
DWORD TAesClass::OnAesUncrypt(LPVOID InBuffer, DWORD InLength,
                              LPVOID OutBuffer) {
  DWORD OutLength = 0;
  if (m_lpAes == NULL || OutBuffer == NULL) {
    return 0;
...
int AesEncrypt(unsigned char* key, char* data, unsigned char* secret_data) {
  TAesClass a;
  //printf("aes encrypt origin:$%s\n", data);
  a.InitializePrivateKey(Bits256, key);
  int length = strlen(data);

  //printf("aes encrypt origin:$len = %d\n", length);

  return a.OnAesEncrypt(data, length+1 , secret_data);

    /*
    if ((length & 31) != 0) {
      length = ((length >> 5) << 5) + 32;
    }

    char* dedata = (char*)malloc(length);
    a.OnAesUncrypt(secret_data, length, dedata);
    printf("%s\n", dedata);
    */
}

void AesDecript(unsigned char* key, unsigned char* secret_data, int length,
                char* data) {
  TAesClass a;
  a.InitializePrivateKey(Bits256, key);

  //std::cout << "密文长度" << length << std::endl;
  ...
  */
  ...
  a.OnAesUncrypt(secret_data, length, data);
}
